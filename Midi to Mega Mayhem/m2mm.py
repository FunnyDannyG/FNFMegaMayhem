...
with open("notes.txt", "w", encoding="UTF-16") as f:
    ...
    ):  # open any channels in the midi, FUCKS UP HARD if there are more than one channel but who would do that for FNF vocals???
        for (
            note
        ) in (
            instrument.notes
        ):  # run the following code for every note found in the current channel
            print(
                GetFNFLyric(librosa.midi_to_note(note.pitch))
            )  # print out the vowel into the terminal
            f.write(
                GetFNFLyric(librosa.midi_to_note(note.pitch))
            )  # append the vowel to a text file
